...
-   **Atomic Build System:** The project uses a multi-stage build process. `process.ts` standardizes raw SVGs into 48x48 tiles with consistent padding and corner radii, while `build.ts` compiles these into a single `icons.json` to eliminate filesystem I/O at runtime.
    ...
    - **Lazy-loaded Icon Manifest:** `icons.json` is not bundled into the Worker source; instead, it is stored in Supabase storage and fetched on first request, then cached in-memory and via `caches.default`. This keeps the Worker bundle under the 3 MiB free-tier limit while preserving the full exhaustive icon set and enabling instant icon grid generation.
